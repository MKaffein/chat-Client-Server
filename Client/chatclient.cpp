...
void ChatClient::jsonReceived(const QJsonObject &docObj)
{
    ...
    if (typeVal.toString().compare(QLatin1String("login"), Qt::CaseInsensitive) == 0) {
        ...
        QString message = QString("%1: %2").arg(sender, text);


        emit displayMessage(message, sender);
...
void ChatClient::onMessageReceived(const QString &sender, const QString &text)
{
    QString message = QString("%1: %2").arg(sender, text);
    emit displayMessage(message, sender);
}
