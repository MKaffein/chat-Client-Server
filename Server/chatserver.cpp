...
void ChatServer::incomingConnection(qintptr socketDescriptor)
{
    ...
    emit logMessage(QStringLiteral("Подключён новый клиент"));
}
...
{
    Q_ASSERT(sender);
    emit logMessage(QStringLiteral("Получил JSON ") + QString::fromUtf8(QJsonDocument(doc).toJson()));
...
void ChatServer::userDisconnected(ServerWorker *sender)
{
    ...
    if (!userName.isEmpty()) {
        ...
        emit logMessage(userName + QStringLiteral(" disconnected"));
    }
    sender->deleteLater();
}
...
{
    Q_UNUSED(sender)
    emit logMessage(QStringLiteral("Ошибка из-за ") + sender->userName());
}
...
void ChatServer::jsonFromLoggedOut(ServerWorker *sender, const QJsonObject &docObj)
{
    ...
    if (typeVal.isNull() || !typeVal.isString())
        return;
    if (typeVal.toString().compare(QStringLiteral("login"), Qt::CaseInsensitive) != 0)
        return;
    const QJsonValue usernameVal = docObj.value(QStringLiteral("username"));
...
void ChatServer::jsonFromLoggedIn(ServerWorker *sender, const QJsonObject &docObj)
{
    ...
    if (typeVal.isNull() || !typeVal.isString())
        return;
    if (typeVal.toString().compare(QStringLiteral("message"), Qt::CaseInsensitive) != 0)
        return;
